...
async function runPerformanceAudit() {
  ...
  try {
    ...
    const warnings = [];
    if (scores.performance < 90) warnings.push(`Performance score basso: ${scores.performance}%`);
    if (scores.accessibility < 95) warnings.push(`Accessibility score basso: ${scores.accessibility}%`);
    if (scores.bestPractices < 90) warnings.push(`Best practices score basso: ${scores.bestPractices}%`);
    ...
    const overallGood = scores.performance >= 90 && scores.accessibility >= 95 && scores.bestPractices >= 90 && scores.seo >= 95;
...
async function main() {
  ...
  if (!serverRunning) {
    ...
    console.log('💡 Assicurati che il server sia in esecuzione con: npm run dev o npm run start');
    process.exit(1);
  }
