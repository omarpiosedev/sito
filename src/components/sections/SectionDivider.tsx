...
export default function SectionDivider({
  ...
  return (
    <section className={`${height} ${backgroundColor} relative overflow-hidden flex items-center justify-center -mt-1 md:mt-32 lg:mt-48 ${containerClassName}`}>
      ...
      <div className="absolute inset-0 pointer-events-none">
        {/* Fade sinistro */}
        <div className={`absolute left-0 top-0 w-20 h-full bg-gradient-to-r from-current to-transparent opacity-100`}
             style={{ background: `linear-gradient(to right, ${backgroundColor === 'bg-black' ? 'black' : 'white'}, transparent)` }}></div>
        {/* Fade destro */}
        <div className={`absolute right-0 top-0 w-20 h-full bg-gradient-to-l from-current to-transparent opacity-100`}
             style={{ background: `linear-gradient(to left, ${backgroundColor === 'bg-black' ? 'black' : 'white'}, transparent)` }}></div>
      </div>
    </section>
  );
}