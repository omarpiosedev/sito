...
const projectsData: Project[] = [
  {
    ...
    description: 'A comprehensive design system and branding project focused on creating bold, modern interfaces with exceptional user experiences.',
  ...
  },
  {
    ...
    description: 'Modern web application built with cutting-edge design principles, featuring seamless user interactions and innovative visual storytelling.',
  ...
  },
  {
    ...
    description: 'Luxury fashion brand identity with sophisticated design approach, creating an elevated brand experience for discerning customers.',
  ...
  },
  {
    ...
    description: 'Innovative product design and branding solution with focus on user-centric approach and contemporary aesthetic principles.',
...
function ProjectCard({ project, index, isReversed = false }: ProjectCardProps) {
  ...
  const rotateRaw = useTransform(scrollYProgress, [0, 1], 
    isLargeScreen 
      ? (isReversed ? [-8, 0] : [8, 0]) 
      : (isReversed ? [-6, 0] : [6, 0])); // Rotazione forte anche su mobile
  
  const xRaw = useTransform(scrollYProgress, [0, 1], 
    isLargeScreen 
      ? (isReversed ? [-50, 25] : [50, -25]) 
      : (isReversed ? [-30, 15] : [30, -15])); // Movimento X drammatico
  
  const yRaw = useTransform(scrollYProgress, [0, 1], 
    isLargeScreen 
      ? (isReversed ? [-50, 25] : [-50, 25]) 
      : [-30, 15]); // Movimento Y drammatico
  ...
  const textXRaw = useTransform(scrollYProgress, [0, 1], 
    isLargeScreen 
      ? (isReversed ? [60, 0] : [-60, 0])
      : (isReversed ? [40, 0] : [-40, 0])); // Movimento testo drammatico
  
  const textOpacityRaw = useTransform(scrollYProgress, [0, 0.5], [0, 1]);
  
  const textX = useSpring(textXRaw, { damping: 20, stiffness: 100 });
  const textOpacity = useSpring(textOpacityRaw, { damping: 20, stiffness: 100 });
  ...
  return (
    ...
    >
      <div className="w-full max-w-full mx-auto">
        <div className={`grid grid-cols-1 lg:grid-cols-5 gap-4 md:gap-6 lg:gap-8 items-center`}>
          
          {/* Text Content */}
          <motion.div 
            className={`space-y-3 md:space-y-4 lg:space-y-6 lg:col-span-2 text-center lg:text-left ${
              isReversed ? 'lg:col-start-4 lg:order-2 lg:text-right' : 'lg:col-start-1 lg:order-1'
            }`}
            style={{
              x: textX,
              opacity: textOpacity
          ...
          >
            {/* Project Number */}
            <div className="text-orange-500 text-base md:text-lg font-light tracking-wider">
              / {project.number}
            </div>

            {/* Project Title */}
            <h2 className="text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold tracking-tight leading-none"
                style={{
            ...
            {/* Project Description */}
            {project.description && (
              <p className={`text-white/80 text-sm md:text-base lg:text-lg font-light leading-relaxed max-w-md mx-auto lg:mx-0 ${
            ...
            <div>
              <button 
                className="bg-orange-500 hover:bg-orange-600 text-white font-medium px-6 md:px-8 py-2 md:py-3 rounded-full transition-all duration-300 ease-out hover:scale-105 inline-flex items-center justify-center text-sm md:text-base"
              >
                VIEW PROJECT
              </button>
            </div>
          ...
          >
            <div className="w-full h-[300px] md:h-[400px] lg:h-[600px] bg-gradient-to-br from-orange-900/20 to-orange-500/10 rounded-2xl overflow-hidden shadow-2xl">
              <Image
                src={project.image}
                alt={project.title}
                width={800}
...
function ProjectsGrid() {
  ...
  const { scrollYProgress } = useScroll({
    target: ref,
    offset: isLargeScreen ? ["start end", "end start"] : ["start end", "center center"]
  });
  ...
  const gridImages = [
    ...
    '/projects/modivo.svg',
    '/logo-p-center.png', // Logo P al centro
    '/projects/nexora.svg',
    '/projects/clar-co.svg',
    '/projects/modivo.svg',
    '/projects/boldcraft.svg'
  ...
  const centerRowY = useTransform(scrollYProgress, [0, 0.3, 0.6, 1], [-100, -50, 0, 50]);
  const centerRowYSpring = useSpring(centerRowY, { damping: 80, stiffness: 400 });
  ...
  const mobileScaleSpring = useSpring(mobileScale, { damping: 70, stiffness: 300 });
  ...
  const centerColumnY = useTransform(scrollYProgress, 
    isLargeScreen 
      ? [0, 0.15, 0.4, 0.6, 1]         // Desktop: valori originali
      : [0, 0.2, 0.6, 1, 1],           // Mobile: completa al centro
    isLargeScreen 
      ? [-250, -150, -50, 47, 47]      // Desktop: offset finale 47
      : [-250, -150, -50, 0, 0]        // Mobile: centrato perfetto
  );
  const centerColumnYSpring = useSpring(centerColumnY, { damping: 80, stiffness: 400, mass: 0.2 });

  const leftColumnWidth = useTransform(scrollYProgress, 
    isLargeScreen 
      ? [0, 0.4, 0.8]                  // Desktop: timing originale
      : [0, 0.5, 1],                   // Mobile: completa al centro
    ["42%", "35%", "33.33%"]
  );
  const rightColumnWidth = useTransform(scrollYProgress, 
    isLargeScreen 
      ? [0, 0.4, 0.8]                  // Desktop: timing originale
      : [0, 0.5, 1],                   // Mobile: completa al centro
    ["42%", "35%", "33.33%"]
  );
  const centerColumnWidth = useTransform(scrollYProgress, 
    isLargeScreen 
      ? [0, 0.4, 0.8]                  // Desktop: timing originale
      : [0, 0.5, 1],                   // Mobile: completa al centro
    ["16%", "30%", "33.33%"]
  ...
  const leftWidthSpring = useSpring(leftColumnWidth, { damping: 75, stiffness: 350, mass: 0.25 });
  const rightWidthSpring = useSpring(rightColumnWidth, { damping: 75, stiffness: 350, mass: 0.25 });
  const centerWidthSpring = useSpring(centerColumnWidth, { damping: 75, stiffness: 350, mass: 0.25 });

  const desktopScale = useTransform(scrollYProgress, [0, 0.15, 0.25, 0.35, 0.45, 0.5, 1], [2.8, 2.2, 1.6, 1.2, 1.05, 1, 1]);
  const desktopScaleSpring = useSpring(desktopScale, { damping: 70, stiffness: 300, mass: 0.3 });

  if (!isLargeScreen) {
    // Layout mobile: stessa animazione del desktop ma ottimizzata per mobile
    return (
      <div ref={ref} className="w-full h-screen flex items-center justify-center overflow-hidden relative">
        <motion.div 
        ...
        <div className="absolute bottom-0 left-0 right-0 h-64 pointer-events-none z-20" 
             style={{
               background: 'linear-gradient(to top, #000000 0%, rgba(0,0,0,0.95) 15%, rgba(0,0,0,0.85) 25%, rgba(0,0,0,0.7) 35%, rgba(0,0,0,0.5) 45%, rgba(0,0,0,0.3) 60%, rgba(0,0,0,0.15) 75%, rgba(0,0,0,0.05) 85%, transparent 100%)'
             }}></div>
      </div>
    );
  }
